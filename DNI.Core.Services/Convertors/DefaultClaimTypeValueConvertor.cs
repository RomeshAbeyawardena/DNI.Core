namespace DNI.Core.Services.Convertors
{
    ...
    {
        private readonly ISwitch<Type, string> valueTypeDictionary;
        private readonly ISwitch<string, Func<string, object>> valueTypeConvertor;

        public DefaultClaimTypeValueConvertor()
        {
            valueTypeDictionary = Switch.Create<Type, string>()
            ...
            valueTypeConvertor = Switch.Create<string, Func<string, object>>()
        ...
        {
            return valueTypeConvertor.Case(claimTypeValue)?.Invoke(value);
        }

        public string GetClaimTypeValue(Type type)
        {
            return valueTypeDictionary.Case(type);
        }
    }
}
